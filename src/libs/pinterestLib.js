...
export const pinterestLogin = (callback) =>
  window.PDK.login({ scope: "read_public, write_public" }, callback);
export const pinterestLogout = () => window.PDK.logout();
export const isLoggedIn = () => !!window.PDK.getSession();
export const getAccessToken = () => isLoggedIn() ? window.PDK.getSession().accessToken : null;
